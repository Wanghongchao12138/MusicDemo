...
// 歌词
@property (weak, nonatomic) IBOutlet TestLb *lrcLabel;
/** 当前播放时间的label */
...
-(NSArray *)allMusics {
    ...
    if (_allMusics == nil) {
        _allMusics = [WHCMusicModel mj_objectArrayWithFilename:@"mlist.plist"];
    }
    return _allMusics;
}
...
-(void)viewDidLayoutSubviews {
    ...
    [self.lrcScrollView setContentSize:CGSizeMake(0, self.allLrcLines.count *kWHCLineHeight)];
...
- (IBAction)clickPlay {
    ...
    WHCMusicModel *model = self.allMusics[self.index];
    
    [[WHCMusicTool shareMusicTool] playMusicWithMusicName:model.mp3];
    ...
    self.totalTimeLabel.text = [[WHCMusicTool shareMusicTool] durationMusicString];
    self.allLrcLines = [WHCLyricTool lyricListWithName:model.lrc];
...
- (IBAction)clickPause {
    ...
    [[WHCMusicTool shareMusicTool] pause];
...
-(void)updatePerSecond{
    ...
    self.currentTimeLabel.text = [[WHCMusicTool shareMusicTool] currentTimeString];
    
    //    self.progressView.progress = [[CZMusicTool shareMusicTool] musicProgress];
    
    NSTimeInterval currentTime = [[WHCMusicTool shareMusicTool] currentTime];
    
    self.progressView.progress = [[WHCMusicTool shareMusicTool] musicProgress];
    
    
    for (int i = 0; i < self.allLrcLines.count; i++) {
        
        WHCLrcModel *cureetModel = self.allLrcLines[i];
        
        WHCLrcModel *nextModel = nil;
...
//            self.lrcLabel.progress =  (currentTime-cureetModel.time)/(nextModel.time - cureetModel.time) * 100;
//
//            for (id objec in self.lrcScrollView.subviews) {
//
//                if ([objec isKindOfClass:[TestLb class]]) {
//                    TestLb *Label = (TestLb *)objec;
...
//            }
    ...
    [self.lrcScrollView setContentOffset:CGPointMake(0, -100 + kWHCLineHeight * self.currentLrcIndex)];
}
// 更新全屏歌词
-(void)updateLrcLineLabelForScrollView {
    
    for (id obje in self.lrcScrollView.subviews) {
        if ([obje isKindOfClass:[TestLb class]]) {
    ...
    for (int i = 0; i < self.allLrcLines.count; i++) {
        ...
        WHCLrcModel *lrcModel = self.allLrcLines[i];
        
        TestLb *label = [[TestLb alloc]init];
        ...
        NSArray *labelV = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-margin-[label]" options:0 metrics:@{@"margin" : @(kWHCLineHeight * i)} views:@{@"label" : label}];
...
-(void)updateScreenMusicInfo {
    ...
    WHCMusicModel *music = self.allMusics[_index];
    ...
    //设置当前时间
    dict[MPNowPlayingInfoPropertyElapsedPlaybackTime]=@([[WHCMusicTool shareMusicTool] currentTime]);
    
    //总时间
    dict[MPMediaItemPropertyPlaybackDuration]= @([[WHCMusicTool shareMusicTool]durationMusic]);
